...
const postgresConfig = {
  ...
  user: 'waynebanks', // replace this with your username
...
findBookAuthors.then(function bookAuthors(){
  assert.deepEqual(books.authors.length, 15)
  assert.deepEqual(authors[0].last_name, 'Alcott')
  assert.deepEqual(authors[1].first_name, 'Paulette')
}).catch(error => {
  console.log('Whoops, my joint is bugging out, b', error);
});
/* --------End of Exercise 4---------------- */
...
let authorIdWithTwoBooks= db.any(SELECT twoBooks.author_id FROM 
  (SELECT author_id, count(author_id) AS total FROM authors 
  JOIN books 
  ON authors.id = books.author_id 
  GROUP BY author_id) twoBooks WHERE total = 2);

authorIdWithTwoBooks.then(function twoBookId(){
  assert.deepEqual(twoBookId.length, 2)
  assert.deepEqual(twoBookId[0].authors.id, 1809)
  assert.deepEqual(twoBookId[1].authors.id, 7805)
}).catch(error => {
  console.log('Whoops, my joint is bugging out, b', error);
});
...
let bookTitlesWithMultipleEditions = db.any(SELECT author_name
FROM books
GROUP BY author_name
HAVING COUNT(distinct page_count) > 1
);

bookTitlesWithMultipleEditions.then( function multipleEditions(){
  assert.deepEqual(multipleEditions.length, 5)
  assert.deepEqual(multipleEditions[0].title, 'The Shining')
  assert.deepEqual(multipleEditions[1].title, 'The Cat in the Hat')
...
/* --------End of Exercise 6---------------- */
 ...
 let findStockedBooks = db.any(SELECT title, first_name, last_name, is_stocked
...
findStockedBooks.then(function(){
  assert.deepEqual(multipleEditions.length, 3)
  assert.deepEqual(multipleEditions[0].title, 'Dune')
  assert.deepEqual(multipleEditions[0].last_name, 'Herbert')
  assert.deepEqual(multipleEditions[2].title, 'The Cat in the Hat')
